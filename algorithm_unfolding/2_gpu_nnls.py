...
class NNLS_model(nn.Module):
    ...
    def forward(self, s):
        if self.first:
            ...
            ones_tensor = torch.ones(self.x_size, self.s_size, device=device)
            s = (ones_tensor.t() -
                 self.eta * ((self.A.weight.t() @ (self.A.weight @ ((ones_tensor ** self.L).t()) - self.x.t()) * ((ones_tensor ** (self.L - 1)).t()))))
        else:
