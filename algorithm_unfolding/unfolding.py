...
#             loss = loss_fn(H@(s_hat.t()), H@(b_s.t()))/(torch.norm(b_x, float('inf'))) #this loss is not good in this unrolling!
...
class NNLS_model(nn.Module):
    def _init_(self, x_size, s_size, L, first=False):
        super(NNLS_model, self)._init_()
        self.x_size = x_size
        self.s_size = s_size
        self.L = L
        self.first = first
...
#         return s
    ...
    def forward(self, s):
        if self.first:
            s = (torch.ones(self.x.shape[0], self.s_size).t()-((self.A.weight.t() @ (self.A.weight @ ((torch.ones(self.x.shape[0], self.s_size) * (self.L)).t())- self.x.t()) * ((torch.ones(self.x.shape[0], self.s_size) * ((self.L) - 1)).t()))))
        else:
            s = s -(self.A.weight.t() @ (self.A.weight @ ((s * (self.L))) - self.x.t()) * ((s * ((self.L) - 1))))
        return s


class nnls_model(nn.Module):
    def _init_(self, x_size, s_size, L):
        super(nnls_model, self)._init_()
...
#     H = 1*(H / torch.norm(H, p='fro'))
    ...
    train_loss = unfolded_nnls_apply(x_size, s_size, train_loader, num_epochs=24000, L=L)
